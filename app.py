...
st.write("VERSIONE 1.0, made by Antonio Foderaro")
...
if st.button("Elabora"):
    ...
    # SPINNER + MESSAGGIO LAMPEGGIANTE
    with st.spinner("⏳ Attendere prego... elaborazione in corso..."):
        placeholder = st.empty()
        for i in range(6):
            placeholder.markdown("### 🔵 **Attendere prego...**")
            time.sleep(0.4)
            placeholder.markdown("### ⚪ **Attendere prego...**")
            time.sleep(0.4)
    ...
    if r.returncode != 0 or not os.path.exists(raw):
        st.error("Errore download. Possibile blocco firewall o server lento.")
        st.stop()
